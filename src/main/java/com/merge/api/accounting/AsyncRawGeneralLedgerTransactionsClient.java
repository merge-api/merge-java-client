...
public class AsyncRawGeneralLedgerTransactionsClient {
    protected final ClientOptions clientOptions;

    public AsyncRawGeneralLedgerTransactionsClient(ClientOptions clientOptions) {
    ...
    public CompletableFuture<MergeApiHttpResponse<SyncPagingIterable<GeneralLedgerTransaction>>> list(
        ...
        if (request.getPostedDateAfter().isPresent()) {
            QueryStringMapper.addQueryParameter(
                    httpUrl, "posted_date_after", request.getPostedDateAfter().get(), false);
        }
        if (request.getPostedDateBefore().isPresent()) {
            QueryStringMapper.addQueryParameter(
                    httpUrl, "posted_date_before", request.getPostedDateBefore().get(), false);
        }
        ...
        CompletableFuture<MergeApiHttpResponse<SyncPagingIterable<GeneralLedgerTransaction>>> future =
            ...
            public void onResponse(@NotNull Call call, @NotNull Response response) throws IOException {
                try (ResponseBody responseBody = response.body()) {
                    if (response.isSuccessful()) {
                        ...
                        GeneralLedgerTransactionsListRequest nextRequest =
                                GeneralLedgerTransactionsListRequest.builder()
                                        .from(request)
                                        .cursor(startingAfter)
                                        .build();
                        List<GeneralLedgerTransaction> result =
     ...
     */
    public CompletableFuture<MergeApiHttpResponse<GeneralLedgerTransaction>> retrieve(String id) {
    ...
    public CompletableFuture<MergeApiHttpResponse<GeneralLedgerTransaction>> retrieve(
        ...
        CompletableFuture<MergeApiHttpResponse<GeneralLedgerTransaction>> future = new CompletableFuture<>();
        client.newCall(okhttpRequest).enqueue(new Callback() {
            @Override
            public void onResponse(@NotNull Call call, @NotNull Response response) throws IOException {
                try (ResponseBody responseBody = response.body()) {
                    if (response.isSuccessful()) {
                        future.complete(new MergeApiHttpResponse<>(
                                ObjectMappers.JSON_MAPPER.readValue(
                                        responseBody.string(), GeneralLedgerTransaction.class),
                                response));
                        return;
                    }
                    String responseBodyString = responseBody != null ? responseBody.string() : "{}";
