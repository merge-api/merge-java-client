...
public class AsyncRawCompanyInfoClient {
    protected final ClientOptions clientOptions;

    public AsyncRawCompanyInfoClient(ClientOptions clientOptions) {
    ...
    public CompletableFuture<MergeApiHttpResponse<SyncPagingIterable<CompanyInfo>>> list(
        ...
        CompletableFuture<MergeApiHttpResponse<SyncPagingIterable<CompanyInfo>>> future = new CompletableFuture<>();
        client.newCall(okhttpRequest).enqueue(new Callback() {
            @Override
            public void onResponse(@NotNull Call call, @NotNull Response response) throws IOException {
                try (ResponseBody responseBody = response.body()) {
                    if (response.isSuccessful()) {
                        PaginatedCompanyInfoList parsedResponse = ObjectMappers.JSON_MAPPER.readValue(
                                responseBody.string(), PaginatedCompanyInfoList.class);
                        Optional<String> startingAfter = parsedResponse.getNext();
                        CompanyInfoListRequest nextRequest = CompanyInfoListRequest.builder()
                                .from(request)
                                .cursor(startingAfter)
                                .build();
                        List<CompanyInfo> result = parsedResponse.getResults().orElse(Collections.emptyList());
                        future.complete(new MergeApiHttpResponse<>(
                                new SyncPagingIterable<CompanyInfo>(startingAfter.isPresent(), result, () -> {
                                    try {
     ...
     */
    public CompletableFuture<MergeApiHttpResponse<CompanyInfo>> retrieve(String id) {
    ...
    public CompletableFuture<MergeApiHttpResponse<CompanyInfo>> retrieve(
            String id, CompanyInfoRetrieveRequest request) {
    ...
    public CompletableFuture<MergeApiHttpResponse<CompanyInfo>> retrieve(
        ...
        CompletableFuture<MergeApiHttpResponse<CompanyInfo>> future = new CompletableFuture<>();
        client.newCall(okhttpRequest).enqueue(new Callback() {
            @Override
            public void onResponse(@NotNull Call call, @NotNull Response response) throws IOException {
                try (ResponseBody responseBody = response.body()) {
                    if (response.isSuccessful()) {
                        future.complete(new MergeApiHttpResponse<>(
                                ObjectMappers.JSON_MAPPER.readValue(responseBody.string(), CompanyInfo.class),
                                response));
                        return;
                    }
                    String responseBodyString = responseBody != null ? responseBody.string() : "{}";
