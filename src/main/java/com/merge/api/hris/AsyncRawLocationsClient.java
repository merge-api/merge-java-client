...
public class AsyncRawLocationsClient {
     ...
     * Returns a list of <code>Location</code> objects.
     */
    public CompletableFuture<MergeApiHttpResponse<SyncPagingIterable<Location>>> list(LocationsListRequest request) {
    ...
    public CompletableFuture<MergeApiHttpResponse<SyncPagingIterable<Location>>> list(
        ...
        CompletableFuture<MergeApiHttpResponse<SyncPagingIterable<Location>>> future = new CompletableFuture<>();
        client.newCall(okhttpRequest).enqueue(new Callback() {
            @Override
            public void onResponse(@NotNull Call call, @NotNull Response response) throws IOException {
                try (ResponseBody responseBody = response.body()) {
                    if (response.isSuccessful()) {
                        PaginatedLocationList parsedResponse =
                                ObjectMappers.JSON_MAPPER.readValue(responseBody.string(), PaginatedLocationList.class);
                        Optional<String> startingAfter = parsedResponse.getNext();
                        LocationsListRequest nextRequest = LocationsListRequest.builder()
                                .from(request)
                                .cursor(startingAfter)
                                .build();
                        List<Location> result = parsedResponse.getResults().orElse(Collections.emptyList());
                        future.complete(new MergeApiHttpResponse<>(
                                new SyncPagingIterable<Location>(startingAfter.isPresent(), result, () -> {
                                    try {
