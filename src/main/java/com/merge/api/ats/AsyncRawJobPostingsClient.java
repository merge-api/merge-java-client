...
public class AsyncRawJobPostingsClient {
    ...
    public CompletableFuture<MergeApiHttpResponse<SyncPagingIterable<JobPosting>>> list(
        ...
        CompletableFuture<MergeApiHttpResponse<SyncPagingIterable<JobPosting>>> future = new CompletableFuture<>();
        client.newCall(okhttpRequest).enqueue(new Callback() {
            @Override
            public void onResponse(@NotNull Call call, @NotNull Response response) throws IOException {
                try (ResponseBody responseBody = response.body()) {
                    if (response.isSuccessful()) {
                        PaginatedJobPostingList parsedResponse = ObjectMappers.JSON_MAPPER.readValue(
                                responseBody.string(), PaginatedJobPostingList.class);
                        Optional<String> startingAfter = parsedResponse.getNext();
                        JobPostingsListRequest nextRequest = JobPostingsListRequest.builder()
                                .from(request)
                                .cursor(startingAfter)
                                .build();
                        List<JobPosting> result = parsedResponse.getResults().orElse(Collections.emptyList());
                        future.complete(new MergeApiHttpResponse<>(
                                new SyncPagingIterable<JobPosting>(startingAfter.isPresent(), result, () -> {
                                    try {
