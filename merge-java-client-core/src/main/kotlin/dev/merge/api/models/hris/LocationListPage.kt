...
private constructor(
    ...
    override fun toString() =
        "LocationListPage{locationsService=$locationsService, params=$params, response=$response}"
    ...
    fun getNextPage(): Optional<LocationListPage> {
        return getNextPageParams().map { locationsService.list(it) }
    }
    ...
    companion object {
        ...
        @JvmStatic
        fun of(locationsService: LocationService, params: LocationListParams, response: Response) =
            LocationListPage(
                locationsService,
                params,
                response,
    ...
    constructor(
        ...
        fun results(): List<Location> = results.getNullable("results") ?: listOf()
        ...
        override fun toString() =
            "LocationListPage.Response{next=$next, previous=$previous, results=$results, additionalProperties=$additionalProperties}"
    ...
    class AutoPager
    constructor(
        private val firstPage: LocationListPage,
    ) : Iterable<Location> {

        override fun iterator(): Iterator<Location> = iterator {
            var page = firstPage
            var index = 0
            while (true) {
