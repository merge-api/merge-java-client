 ...
 * ### Description
 *
 * The `BalanceSheet` object shows a company’s assets, liabilities, and equity. Assets should be
 * equal to liability and equity combined. This shows the company’s financial health at a specific
 * point in time.
 *
 * ### Usage Example
 *
...
private constructor(
    ...
    private val netAssets: JsonField<Double>,
    private val assets: JsonField<List<ReportItem>>,
    private val liabilities: JsonField<List<ReportItem>>,
    private val equity: JsonField<List<ReportItem>>,
    ...
    /**
     * The balance sheet's date. The balance sheet data will reflect the company's financial
     * position this point in time.
     */
    fun date(): Optional<OffsetDateTime> = Optional.ofNullable(date.getNullable("date"))

    /** The balance sheet's net assets. */
    fun netAssets(): Optional<Double> = Optional.ofNullable(netAssets.getNullable("net_assets"))

    fun assets(): Optional<List<ReportItem>> = Optional.ofNullable(assets.getNullable("assets"))

    fun liabilities(): Optional<List<ReportItem>> =
        Optional.ofNullable(liabilities.getNullable("liabilities"))

    fun equity(): Optional<List<ReportItem>> = Optional.ofNullable(equity.getNullable("equity"))
    ...
    /** The balance sheet's net assets. */
    @JsonProperty("net_assets") @ExcludeMissing fun _netAssets() = netAssets

    @JsonProperty("assets") @ExcludeMissing fun _assets() = assets

    @JsonProperty("liabilities") @ExcludeMissing fun _liabilities() = liabilities
    ...
    fun validate(): BalanceSheet = apply {
        if (!validated) {
            ...
            date()
            netAssets()
            assets().map { it.forEach { it.validate() } }
            liabilities().map { it.forEach { it.validate() } }
            equity().map { it.forEach { it.validate() } }
            remoteGeneratedAt()
            remoteWasDeleted()
            modifiedAt()
    ...
    override fun equals(other: Any?): Boolean {
        ...
        return other is BalanceSheet &&
            ...
            this.netAssets == other.netAssets &&
            this.assets == other.assets &&
            this.liabilities == other.liabilities &&
    ...
    override fun toString() =
        "BalanceSheet{id=$id, remoteId=$remoteId, name=$name, currency=$currency, company=$company, date=$date, netAssets=$netAssets, assets=$assets, liabilities=$liabilities, equity=$equity, remoteGeneratedAt=$remoteGeneratedAt, remoteWasDeleted=$remoteWasDeleted, fieldMappings=$fieldMappings, modifiedAt=$modifiedAt, remoteData=$remoteData, additionalProperties=$additionalProperties}"
    ...
    class Builder {
        ...
        private var netAssets: JsonField<Double> = JsonMissing.of()
        private var assets: JsonField<List<ReportItem>> = JsonMissing.of()
        private var liabilities: JsonField<List<ReportItem>> = JsonMissing.of()
        private var equity: JsonField<List<ReportItem>> = JsonMissing.of()
        ...
        @JvmSynthetic
        internal fun from(balanceSheet: BalanceSheet) = apply {
            this.id = balanceSheet.id
            this.remoteId = balanceSheet.remoteId
            this.name = balanceSheet.name
            this.currency = balanceSheet.currency
            this.company = balanceSheet.company
            this.date = balanceSheet.date
            this.netAssets = balanceSheet.netAssets
            this.assets = balanceSheet.assets
            this.liabilities = balanceSheet.liabilities
            this.equity = balanceSheet.equity
            this.remoteGeneratedAt = balanceSheet.remoteGeneratedAt
            this.remoteWasDeleted = balanceSheet.remoteWasDeleted
            this.fieldMappings = balanceSheet.fieldMappings
            this.modifiedAt = balanceSheet.modifiedAt
            this.remoteData = balanceSheet.remoteData
            additionalProperties(balanceSheet.additionalProperties)
        }
        ...
        /** The balance sheet's net assets. */
        fun netAssets(netAssets: Double) = netAssets(JsonField.of(netAssets))

        /** The balance sheet's net assets. */
        @JsonProperty("net_assets")
        @ExcludeMissing
        fun netAssets(netAssets: JsonField<Double>) = apply { this.netAssets = netAssets }

        fun assets(assets: List<ReportItem>) = assets(JsonField.of(assets))

        @JsonProperty("assets")
        @ExcludeMissing
        fun assets(assets: JsonField<List<ReportItem>>) = apply { this.assets = assets }

        fun liabilities(liabilities: List<ReportItem>) = liabilities(JsonField.of(liabilities))

        @JsonProperty("liabilities")
        @ExcludeMissing
        fun liabilities(liabilities: JsonField<List<ReportItem>>) = apply {
            this.liabilities = liabilities
        }

        fun equity(equity: List<ReportItem>) = equity(JsonField.of(equity))

        @JsonProperty("equity")
        @ExcludeMissing
        fun equity(equity: JsonField<List<ReportItem>>) = apply { this.equity = equity }
        ...
        fun build(): BalanceSheet =
            BalanceSheet(
                ...
                date,
                netAssets,
                assets.map { it.toUnmodifiable() },
                liabilities.map { it.toUnmodifiable() },
                equity.map { it.toUnmodifiable() },
                remoteGeneratedAt,
                remoteWasDeleted,
                fieldMappings,
                modifiedAt,
