...
sealed class JsonField<out T : Any> {
    ...
    fun asStringOrThrow(): String =
        when (this) {
            is JsonString -> value
            else -> throw MergeInvalidDataException("Value is not a string")
        }
    ...
    internal fun getRequired(name: String): T =
        when (this) {
            is KnownValue -> value
            is JsonMissing -> throw MergeInvalidDataException("'${name}' is not set")
            is JsonNull -> throw MergeInvalidDataException("'${name}' is null")
            else -> throw MergeInvalidDataException("'${name}' is invalid, received ${this}")
        }
