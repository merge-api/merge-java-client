...
private constructor(
    ...
    override fun toString() =
        "TicketListPageAsync{ticketsService=$ticketsService, params=$params, response=$response}"
    ...
    fun getNextPage(): CompletableFuture<Optional<TicketListPageAsync>> {
        return getNextPageParams()
            .map { ticketsService.list(it).thenApply { Optional.of(it) } }
    ...
    companion object {
        ...
        @JvmStatic
        fun of(ticketsService: TicketServiceAsync, params: TicketListParams, response: Response) =
            TicketListPageAsync(
                ticketsService,
                params,
                response,
    ...
    constructor(
        ...
        fun results(): List<Ticket> = results.getNullable("results") ?: listOf()
        ...
        override fun toString() =
            "TicketListPageAsync.Response{next=$next, previous=$previous, results=$results, additionalProperties=$additionalProperties}"
    ...
    class AutoPager
    constructor(
        private val firstPage: TicketListPageAsync,
    ) {

        fun forEach(action: Predicate<Ticket>, executor: Executor): CompletableFuture<Void> {
            fun CompletableFuture<Optional<TicketListPageAsync>>.forEach(
                action: (Ticket) -> Boolean,
                executor: Executor
        ...
        fun toList(executor: Executor): CompletableFuture<List<Ticket>> {
            val values = mutableListOf<Ticket>()
