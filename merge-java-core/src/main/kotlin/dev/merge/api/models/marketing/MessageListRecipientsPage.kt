...
class MessageListRecipientsPage
private constructor(
    private val messagesService: MessageService,
    private val params: MessageListRecipientsParams,
    ...
    override fun equals(other: Any?): Boolean {
        ...
        return other is MessageListRecipientsPage &&
            this.messagesService == other.messagesService &&
    ...
    override fun toString() =
        "MessageListRecipientsPage{messagesService=$messagesService, params=$params, response=$response}"
    ...
    fun getNextPageParams(): Optional<MessageListRecipientsParams> {
        if (!hasNextPage()) {
            return Optional.empty()
        }

        return Optional.of(
            MessageListRecipientsParams.builder().from(params).cursor(next()).build()
        )
    }

    fun getNextPage(): Optional<MessageListRecipientsPage> {
        return getNextPageParams().map { messagesService.listRecipients(it) }
    }
    ...
    constructor(
        ...
        override fun toString() =
            "MessageListRecipientsPage.Response{next=$next, previous=$previous, results=$results, additionalProperties=$additionalProperties}"
    ...
    class AutoPager
    constructor(
        private val firstPage: MessageListRecipientsPage,
    ) : Iterable<Contact> {
