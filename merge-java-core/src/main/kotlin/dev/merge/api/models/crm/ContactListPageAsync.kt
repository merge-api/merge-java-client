...
private constructor(
    ...
    override fun toString() =
        "ContactListPageAsync{contactsService=$contactsService, params=$params, response=$response}"
    ...
    fun getNextPageParams(): Optional<ContactListParams> {
        if (!hasNextPage()) {
            return Optional.empty()
        }

        return Optional.of(ContactListParams.builder().from(params).cursor(next()).build())
    }

    fun getNextPage(): CompletableFuture<Optional<ContactListPageAsync>> {
        return getNextPageParams()
            .map { contactsService.list(it).thenApply { Optional.of(it) } }
    ...
    constructor(
        ...
        override fun toString() =
            "ContactListPageAsync.Response{next=$next, previous=$previous, results=$results, additionalProperties=$additionalProperties}"
    ...
    class AutoPager
    constructor(
        private val firstPage: ContactListPageAsync,
    ) {

        fun forEach(action: Predicate<Contact>, executor: Executor): CompletableFuture<Void> {
            fun CompletableFuture<Optional<ContactListPageAsync>>.forEach(
