 ...
 * ### Description
 *
 * The `RejectReason` object is used to represent a reason for rejecting an application. These can
 * typically be configured within an ATS system.
 *
 * ### Usage Example
 *
 * Fetch from the `LIST RejectReasons` endpoint and filter by `ID` to show all reasons.
 */
@JsonDeserialize(builder = RejectReason.Builder::class)
@NoAutoDetect
class RejectReason
private constructor(
    ...
    override fun toString() =
        "RejectReason{id=$id, remoteId=$remoteId, name=$name, remoteWasDeleted=$remoteWasDeleted, fieldMappings=$fieldMappings, modifiedAt=$modifiedAt, remoteData=$remoteData, additionalProperties=$additionalProperties}"
    ...
    class Builder {
        ...
        @JvmSynthetic
        internal fun from(rejectReason: RejectReason) = apply {
            this.id = rejectReason.id
            this.remoteId = rejectReason.remoteId
            this.name = rejectReason.name
            this.remoteWasDeleted = rejectReason.remoteWasDeleted
            this.fieldMappings = rejectReason.fieldMappings
            this.modifiedAt = rejectReason.modifiedAt
            this.remoteData = rejectReason.remoteData
            additionalProperties(rejectReason.additionalProperties)
        }
