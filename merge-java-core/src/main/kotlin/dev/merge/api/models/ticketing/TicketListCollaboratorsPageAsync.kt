...
class TicketListCollaboratorsPageAsync
private constructor(
    private val ticketsService: TicketServiceAsync,
    private val params: TicketListCollaboratorsParams,
    ...
    override fun equals(other: Any?): Boolean {
        ...
        return other is TicketListCollaboratorsPageAsync &&
    ...
    override fun toString() =
        "TicketListCollaboratorsPageAsync{ticketsService=$ticketsService, params=$params, response=$response}"
    ...
    fun getNextPageParams(): Optional<TicketListCollaboratorsParams> {
        if (!hasNextPage()) {
            return Optional.empty()
        }

        return Optional.of(
            TicketListCollaboratorsParams.builder().from(params).cursor(next()).build()
        )
    }

    fun getNextPage(): CompletableFuture<Optional<TicketListCollaboratorsPageAsync>> {
        return getNextPageParams()
            .map { ticketsService.listCollaborators(it).thenApply { Optional.of(it) } }
    ...
    companion object {
        ...
        fun of(
            ...
            TicketListCollaboratorsPageAsync(
                ticketsService,
                params,
                response,
            )
    ...
    constructor(
        ...
        fun results(): List<User> = results.getRequired("results")
        ...
        override fun toString() =
            "TicketListCollaboratorsPageAsync.Response{next=$next, previous=$previous, results=$results, additionalProperties=$additionalProperties}"
    ...
    class AutoPager
    constructor(
        private val firstPage: TicketListCollaboratorsPageAsync,
    ) {

        fun forEach(action: Predicate<User>, executor: Executor): CompletableFuture<Void> {
            fun CompletableFuture<Optional<TicketListCollaboratorsPageAsync>>.forEach(
                action: (User) -> Boolean,
                executor: Executor
