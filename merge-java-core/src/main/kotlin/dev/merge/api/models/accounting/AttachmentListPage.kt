...
private constructor(
    ...
    fun results(): List<AccountingAttachment> = response().results()
    ...
    override fun toString() =
        "AttachmentListPage{attachmentsService=$attachmentsService, params=$params, response=$response}"
    ...
    fun getNextPageParams(): Optional<AttachmentListParams> {
        ...
        return Optional.of(AttachmentListParams.builder().from(params).cursor(next()).build())
    }

    fun getNextPage(): Optional<AttachmentListPage> {
        return getNextPageParams().map { attachmentsService.list(it) }
    }
    ...
    constructor(
        ...
        private val results: JsonField<List<AccountingAttachment>>,
        ...
        fun results(): List<AccountingAttachment> = results.getRequired("results")
        ...
        fun _results(): Optional<JsonField<List<AccountingAttachment>>> =
        ...
        override fun toString() =
            "AttachmentListPage.Response{next=$next, previous=$previous, results=$results, additionalProperties=$additionalProperties}"
        ...
        class Builder {
            ...
            private var results: JsonField<List<AccountingAttachment>> = JsonMissing.of()
            ...
            fun results(results: List<AccountingAttachment>) = results(JsonField.of(results))

            @JsonProperty("results")
            fun results(results: JsonField<List<AccountingAttachment>>) = apply {
    ...
    class AutoPager
    constructor(
        private val firstPage: AttachmentListPage,
    ) : Iterable<AccountingAttachment> {

        override fun iterator(): Iterator<AccountingAttachment> = iterator {
            var page = firstPage
            var index = 0
            while (true) {
        ...
        fun stream(): Stream<AccountingAttachment> {
