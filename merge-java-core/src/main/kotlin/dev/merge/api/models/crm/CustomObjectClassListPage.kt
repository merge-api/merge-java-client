...
class CustomObjectClassListPage
private constructor(
    private val customObjectClassesService: CustomObjectClassService,
    ...
    override fun equals(other: Any?): Boolean {
        ...
        return other is CustomObjectClassListPage &&
    ...
    override fun toString() =
        "CustomObjectClassListPage{customObjectClassesService=$customObjectClassesService, params=$params, response=$response}"
    ...
    fun getNextPageParams(): Optional<CustomObjectClassListParams> {
        ...
        return Optional.of(
            CustomObjectClassListParams.builder().from(params).cursor(next()).build()
        )
    }

    fun getNextPage(): Optional<CustomObjectClassListPage> {
        return getNextPageParams().map { customObjectClassesService.list(it) }
    }
    ...
    companion object {
        ...
        @JvmStatic
        fun of(
            customObjectClassesService: CustomObjectClassService,
    ...
    constructor(
        ...
        fun results(): List<CustomObjectClass> = results.getRequired("results")
        ...
        override fun toString() =
            "CustomObjectClassListPage.Response{next=$next, previous=$previous, results=$results, additionalProperties=$additionalProperties}"
    ...
    class AutoPager
    constructor(
        private val firstPage: CustomObjectClassListPage,
    ) : Iterable<CustomObjectClass> {

        override fun iterator(): Iterator<CustomObjectClass> = iterator {
            var page = firstPage
            var index = 0
            while (true) {
        ...
        fun stream(): Stream<CustomObjectClass> {
