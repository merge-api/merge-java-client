...
class SyncStatusListPageAsync
private constructor(
    private val syncStatusService: SyncStatusServiceAsync,
    ...
    override fun equals(other: Any?): Boolean {
        ...
        return other is SyncStatusListPageAsync &&
    ...
    override fun toString() =
        "SyncStatusListPageAsync{syncStatusService=$syncStatusService, params=$params, response=$response}"
    ...
    fun getNextPageParams(): Optional<SyncStatusListParams> {
        ...
        return Optional.of(SyncStatusListParams.builder().from(params).cursor(next()).build())
    }

    fun getNextPage(): CompletableFuture<Optional<SyncStatusListPageAsync>> {
        return getNextPageParams()
            .map { syncStatusService.list(it).thenApply { Optional.of(it) } }
    ...
    companion object {
        ...
        @JvmStatic
        fun of(
            syncStatusService: SyncStatusServiceAsync,
            params: SyncStatusListParams,
            response: Response
        ) =
            SyncStatusListPageAsync(
                syncStatusService,
                params,
                response,
            )
    ...
    constructor(
        ...
        fun results(): List<SyncStatus> = results.getRequired("results")
        ...
        override fun toString() =
            "SyncStatusListPageAsync.Response{next=$next, previous=$previous, results=$results, additionalProperties=$additionalProperties}"
    ...
    class AutoPager
    constructor(
        private val firstPage: SyncStatusListPageAsync,
    ) {

        fun forEach(action: Predicate<SyncStatus>, executor: Executor): CompletableFuture<Void> {
            fun CompletableFuture<Optional<SyncStatusListPageAsync>>.forEach(
                action: (SyncStatus) -> Boolean,
                executor: Executor
        ...
        fun toList(executor: Executor): CompletableFuture<List<SyncStatus>> {
            val values = mutableListOf<SyncStatus>()
