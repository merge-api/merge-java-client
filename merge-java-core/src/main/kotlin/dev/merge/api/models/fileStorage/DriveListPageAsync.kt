...
class DriveListPageAsync
private constructor(
    private val drivesService: DriveServiceAsync,
    ...
    override fun equals(other: Any?): Boolean {
        ...
        return other is DriveListPageAsync &&
    ...
    override fun toString() =
        "DriveListPageAsync{drivesService=$drivesService, params=$params, response=$response}"
    ...
    fun getNextPage(): CompletableFuture<Optional<DriveListPageAsync>> {
        return getNextPageParams()
            .map { drivesService.list(it).thenApply { Optional.of(it) } }
    ...
    companion object {
        ...
        @JvmStatic
        fun of(drivesService: DriveServiceAsync, params: DriveListParams, response: Response) =
            DriveListPageAsync(
                drivesService,
                params,
                response,
    ...
    constructor(
        ...
        fun results(): List<Drive> = results.getNullable("results") ?: listOf()
        ...
        override fun toString() =
            "DriveListPageAsync.Response{next=$next, previous=$previous, results=$results, additionalProperties=$additionalProperties}"
    ...
    class AutoPager
    constructor(
        private val firstPage: DriveListPageAsync,
    ) {

        fun forEach(action: Predicate<Drive>, executor: Executor): CompletableFuture<Void> {
            fun CompletableFuture<Optional<DriveListPageAsync>>.forEach(
                action: (Drive) -> Boolean,
                executor: Executor
        ...
        fun toList(executor: Executor): CompletableFuture<List<Drive>> {
            val values = mutableListOf<Drive>()
