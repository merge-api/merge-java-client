...
private constructor(
    private val commentsService: CommentServiceAsync,
    ...
    override fun toString() =
        "CommentListPageAsync{commentsService=$commentsService, params=$params, response=$response}"
    ...
    fun getNextPageParams(): Optional<CommentListParams> {
        ...
        return Optional.of(CommentListParams.builder().from(params).cursor(next()).build())
    }

    fun getNextPage(): CompletableFuture<Optional<CommentListPageAsync>> {
        return getNextPageParams()
            .map { commentsService.list(it).thenApply { Optional.of(it) } }
    ...
    companion object {
        ...
        @JvmStatic
        fun of(
            commentsService: CommentServiceAsync,
    ...
    constructor(
        ...
        fun results(): List<Comment> = results.getRequired("results")
        ...
        override fun toString() =
            "CommentListPageAsync.Response{next=$next, previous=$previous, results=$results, additionalProperties=$additionalProperties}"
    ...
    class AutoPager
    constructor(
        private val firstPage: CommentListPageAsync,
    ) {

        fun forEach(action: Predicate<Comment>, executor: Executor): CompletableFuture<Void> {
            fun CompletableFuture<Optional<CommentListPageAsync>>.forEach(
                action: (Comment) -> Boolean,
                executor: Executor
        ...
        fun toList(executor: Executor): CompletableFuture<List<Comment>> {
            val values = mutableListOf<Comment>()
