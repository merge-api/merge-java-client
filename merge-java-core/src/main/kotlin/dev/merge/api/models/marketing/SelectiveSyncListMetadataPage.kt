...
class SelectiveSyncListMetadataPage
private constructor(
    private val selectiveSyncService: SelectiveSyncService,
    ...
    override fun equals(other: Any?): Boolean {
        ...
        return other is SelectiveSyncListMetadataPage &&
    ...
    override fun toString() =
        "SelectiveSyncListMetadataPage{selectiveSyncService=$selectiveSyncService, params=$params, response=$response}"
    ...
    fun getNextPageParams(): Optional<SelectiveSyncListMetadataParams> {
        ...
        return Optional.of(
            SelectiveSyncListMetadataParams.builder().from(params).cursor(next()).build()
        )
    }

    fun getNextPage(): Optional<SelectiveSyncListMetadataPage> {
        return getNextPageParams().map { selectiveSyncService.listMetadata(it) }
    }
    ...
    constructor(
        ...
        fun results(): List<ConditionSchema> = results.getRequired("results")
        ...
        override fun toString() =
            "SelectiveSyncListMetadataPage.Response{next=$next, previous=$previous, results=$results, additionalProperties=$additionalProperties}"
    ...
    class AutoPager
    constructor(
        private val firstPage: SelectiveSyncListMetadataPage,
    ) : Iterable<ConditionSchema> {

        override fun iterator(): Iterator<ConditionSchema> = iterator {
            var page = firstPage
            var index = 0
            while (true) {
        ...
        fun stream(): Stream<ConditionSchema> {
