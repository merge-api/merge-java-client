...
class ContactListListSubscribersPageAsync
private constructor(
    private val contactListsService: ContactListServiceAsync,
    private val params: ContactListListSubscribersParams,
    ...
    override fun equals(other: Any?): Boolean {
        ...
        return other is ContactListListSubscribersPageAsync &&
    ...
    override fun toString() =
        "ContactListListSubscribersPageAsync{contactListsService=$contactListsService, params=$params, response=$response}"
    ...
    fun getNextPageParams(): Optional<ContactListListSubscribersParams> {
        if (!hasNextPage()) {
            return Optional.empty()
        }

        return Optional.of(
            ContactListListSubscribersParams.builder().from(params).cursor(next()).build()
        )
    }

    fun getNextPage(): CompletableFuture<Optional<ContactListListSubscribersPageAsync>> {
        return getNextPageParams()
            .map { contactListsService.listSubscribers(it).thenApply { Optional.of(it) } }
    ...
    companion object {
        ...
        fun of(
            ...
            ContactListListSubscribersPageAsync(
                contactListsService,
                params,
                response,
            )
    ...
    constructor(
        ...
        fun results(): List<Contact> = results.getRequired("results")
        ...
        override fun toString() =
            "ContactListListSubscribersPageAsync.Response{next=$next, previous=$previous, results=$results, additionalProperties=$additionalProperties}"
    ...
    class AutoPager
    constructor(
        private val firstPage: ContactListListSubscribersPageAsync,
    ) {

        fun forEach(action: Predicate<Contact>, executor: Executor): CompletableFuture<Void> {
            fun CompletableFuture<Optional<ContactListListSubscribersPageAsync>>.forEach(
