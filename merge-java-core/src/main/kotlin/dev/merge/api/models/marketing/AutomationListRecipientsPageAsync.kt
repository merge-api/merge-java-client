...
class AutomationListRecipientsPageAsync
private constructor(
    private val automationsService: AutomationServiceAsync,
    private val params: AutomationListRecipientsParams,
    ...
    override fun equals(other: Any?): Boolean {
        ...
        return other is AutomationListRecipientsPageAsync &&
    ...
    override fun toString() =
        "AutomationListRecipientsPageAsync{automationsService=$automationsService, params=$params, response=$response}"
    ...
    fun getNextPageParams(): Optional<AutomationListRecipientsParams> {
        if (!hasNextPage()) {
            return Optional.empty()
        }

        return Optional.of(
            AutomationListRecipientsParams.builder().from(params).cursor(next()).build()
        )
    }

    fun getNextPage(): CompletableFuture<Optional<AutomationListRecipientsPageAsync>> {
        return getNextPageParams()
            .map { automationsService.listRecipients(it).thenApply { Optional.of(it) } }
    ...
    companion object {
        ...
        fun of(
            ...
            AutomationListRecipientsPageAsync(
                automationsService,
                params,
                response,
            )
    ...
    constructor(
        ...
        override fun toString() =
            "AutomationListRecipientsPageAsync.Response{next=$next, previous=$previous, results=$results, additionalProperties=$additionalProperties}"
    ...
    class AutoPager
    constructor(
        private val firstPage: AutomationListRecipientsPageAsync,
    ) {

        fun forEach(action: Predicate<Contact>, executor: Executor): CompletableFuture<Void> {
            fun CompletableFuture<Optional<AutomationListRecipientsPageAsync>>.forEach(
