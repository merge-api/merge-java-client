...
class ProjectListUsersPageAsync
private constructor(
    private val projectsService: ProjectServiceAsync,
    private val params: ProjectListUsersParams,
    ...
    override fun equals(other: Any?): Boolean {
        ...
        return other is ProjectListUsersPageAsync &&
    ...
    override fun toString() =
        "ProjectListUsersPageAsync{projectsService=$projectsService, params=$params, response=$response}"
    ...
    fun getNextPageParams(): Optional<ProjectListUsersParams> {
        if (!hasNextPage()) {
            return Optional.empty()
        }

        return Optional.of(ProjectListUsersParams.builder().from(params).cursor(next()).build())
    }

    fun getNextPage(): CompletableFuture<Optional<ProjectListUsersPageAsync>> {
        return getNextPageParams()
            .map { projectsService.listUsers(it).thenApply { Optional.of(it) } }
    ...
    constructor(
        ...
        override fun toString() =
            "ProjectListUsersPageAsync.Response{next=$next, previous=$previous, results=$results, additionalProperties=$additionalProperties}"
    ...
    class AutoPager
    constructor(
        private val firstPage: ProjectListUsersPageAsync,
    ) {

        fun forEach(action: Predicate<User>, executor: Executor): CompletableFuture<Void> {
            fun CompletableFuture<Optional<ProjectListUsersPageAsync>>.forEach(
                action: (User) -> Boolean,
                executor: Executor
