...
class JournalEntryListPageAsync
private constructor(
    private val journalEntriesService: JournalEntryServiceAsync,
    ...
    override fun equals(other: Any?): Boolean {
        ...
        return other is JournalEntryListPageAsync &&
    ...
    override fun toString() =
        "JournalEntryListPageAsync{journalEntriesService=$journalEntriesService, params=$params, response=$response}"
    ...
    fun getNextPageParams(): Optional<JournalEntryListParams> {
        ...
        return Optional.of(JournalEntryListParams.builder().from(params).cursor(next()).build())
    }

    fun getNextPage(): CompletableFuture<Optional<JournalEntryListPageAsync>> {
        return getNextPageParams()
            .map { journalEntriesService.list(it).thenApply { Optional.of(it) } }
    ...
    companion object {
        ...
        @JvmStatic
        fun of(
            journalEntriesService: JournalEntryServiceAsync,
            params: JournalEntryListParams,
            response: Response
        ) =
            JournalEntryListPageAsync(
                journalEntriesService,
                params,
                response,
            )
    ...
    constructor(
        ...
        fun results(): List<JournalEntry> = results.getRequired("results")
        ...
        override fun toString() =
            "JournalEntryListPageAsync.Response{next=$next, previous=$previous, results=$results, additionalProperties=$additionalProperties}"
    ...
    class AutoPager
    constructor(
        private val firstPage: JournalEntryListPageAsync,
    ) {

        fun forEach(action: Predicate<JournalEntry>, executor: Executor): CompletableFuture<Void> {
            fun CompletableFuture<Optional<JournalEntryListPageAsync>>.forEach(
                action: (JournalEntry) -> Boolean,
                executor: Executor
        ...
        fun toList(executor: Executor): CompletableFuture<List<JournalEntry>> {
            val values = mutableListOf<JournalEntry>()
